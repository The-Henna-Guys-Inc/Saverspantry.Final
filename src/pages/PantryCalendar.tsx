    ...
    <main className="min-h-screen bg-background">
      ...
      <div className="container max-w-6xl mx-auto px-6 py-12">
        <div className="flex items-center gap-2 text-accent text-xs font-semibold uppercase tracking-widest mb-2">
          <CalendarDays className="h-3.5 w-3.5" /> Expiry calendar
        </div>
        <h1 className="text-3xl sm:text-4xl font-bold text-primary mb-2">When your pantry runs out</h1>
        <p className="text-muted-foreground mb-8">A glance at what's expiring — pick a date to see the items.</p>

        {loading ? (
          <div className="flex justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>
        ) : (
          <div className="grid lg:grid-cols-[auto_1fr] gap-6 items-start">
            <Card className="p-3 rounded-3xl border-border/50 shadow-soft inline-block">
              <Calendar
                mode="single"
                selected={selected}
                onSelect={setSelected}
                modifiers={{
                  expiring: expiringDates.soon,
                  expired: expiringDates.expired,
                  any: expiringDates.all,
                }}
                modifiersClassNames={{
                  ...
                  <ul className="space-y-2">
                    {selectedItems.map((it) => (
                      <li key={it.id} className="flex items-center justify-between text-sm">
                        <span className="font-medium text-primary">{it.item}</span>
                        <span className="text-xs text-muted-foreground">{it.quantity} {it.unit}{it.category ? ` · ${it.category}` : ""}</span>
                      </li>
                    ))}
                  </ul>
                )}
                  ...
                  <ul className="space-y-2">
                    {upcoming.map((it) => (
                      <li key={it.id} className="flex items-center justify-between text-sm">
                        ...
                        <Badge variant="secondary" className="rounded-lg">{it.expires_on}</Badge>
                      </li>
                    ))}
                  </ul>
                )}
